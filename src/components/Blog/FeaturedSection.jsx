    ...
    <Link to={`/blog/${category?.toLowerCase()}/${slug}`} className="group block">
      <article className="relative h-full bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-shadow duration-500 border border-black/5">
        ...
        <div className="relative h-[400px] w-full">
          <img
            src="/assets/Images/blogfeatured.png"
            alt={title}
          ...
          <div className="absolute top-0 left-0 h-full w-[45%] p-5 md:p-8 flex flex-col justify-center">
            {/* 1. Category Chip */}
            <div className="mb-3">
              <span className="px-3 py-1 bg-[#15a36e]/10 text-[#15a36e] text-xs font-bold rounded-full backdrop-blur-sm" style={{ fontFamily: 'Palanquin, sans-serif' }}>
                {category}
              </span>
            </div>

            {/* 2. Title Heading (2 lines) */}
            <h3 className="text-lg sm:text-xl lg:text-2xl font-bold text-white mb-3 leading-tight line-clamp-2 group-hover:text-[#15a36e] transition-colors" style={{ fontFamily: 'Palanquin, sans-serif' }}>
              {title}
            </h3>

            {/* 3. Excerpt (3 lines) */}
            <p className="text-sm text-gray-200 leading-relaxed mb-4 line-clamp-3" style={{ fontFamily: 'Palanquin, sans-serif' }}>
              {excerpt}
            </p>

            {/* 4. Date & Read Time */}
            <div className="flex items-center gap-2.5 text-xs text-gray-300 mb-4">
            ...
            {/* 5. Know More Button */}
            <Link to={`/blog/${category?.toLowerCase()}/${slug}`} className="group inline-flex items-center justify-between min-w-[110px] sm:min-w-[125px] bg-[#15a36e] border border-[#15a36e] hover:bg-[#15a36e] rounded-full p-1 transition-all duration-300 w-fit">
              <span className="pl-2.5 pr-2 text-white font-semibold text-xs sm:text-sm" style={{ fontFamily: 'Palanquin, sans-serif' }}>
                Know more
              </span>
              <div className="w-4 h-4 sm:w-5 sm:h-5 rounded-full bg-white flex items-center justify-center transition-all duration-300">
                <svg
                  className="w-2 h-2 sm:w-2.5 sm:h-2.5 text-[#15a36e]"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                  strokeWidth="3"
